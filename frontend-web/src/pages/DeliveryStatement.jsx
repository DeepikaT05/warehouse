              ...
              <div className="border-2 border-slate-200 rounded-2xl p-6 bg-slate-50 space-y-6 font-sans">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-2">
                  <h4 className="font-bold text-xs text-slate-800">
                    Dispatched Box QR List ({selectedDispatch.scannedBoxQrIds?.length || 0} Verified Items)
                  </h4>
                  <div className="flex flex-wrap gap-2 pt-1">
                    {selectedDispatch.scannedBoxQrIds?.map((qr, idx) => (
                      <span key={idx} className="bg-emerald-50 text-[#0F6E56] border border-emerald-200 px-2.5 py-1 rounded-lg text-xs font-mono font-bold">
                        {qr}
                      </span>
                    ))}
                  </div>
                ...
                {selectedDispatch.dispatchPhotoUrl && (
                  <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-2">
                    <h4 className="font-bold text-xs text-slate-800 flex items-center gap-1.5">
                      ...
                      <span>Physical Goods Loading Photo Proof</span>
                    </h4>
                    <img
                      src={selectedDispatch.dispatchPhotoUrl}
                ...
                {/* Signature Blocks */}
                <div className="grid grid-cols-3 gap-4 pt-6 text-[11px] text-slate-600 text-center border-t border-slate-300">
                  <div className="border-t border-slate-400 pt-2 font-bold text-slate-800">
                    Prepared By: {selectedDispatch.verifiedBy}
                  </div>
                  <div className="border-t border-slate-400 pt-2">
                    Driver Signature
                  </div>
                  <div className="border-t border-slate-400 pt-2">
                    Dealer Receiver Signature
                  </div>
                </div>
              </div>
            </div>
